...
function ListarPedidos() {
  ...
  const handleSalvar = async (idPedido: number) => {
    try {
      ...
      // Atualizar os produtos associados
      const produtosAntigos = pedidoOriginal.produtos.map((p: any) => p.idProduto);
      const produtosNovos = formEdicao.produtos.map((p: any) => p.idProduto);

      // Produtos removidos
      const removidos = produtosAntigos.filter((id: any) => !produtosNovos.includes(id));
      for (const idProduto of removidos) {
        await axios.delete(`http://localhost:3000/pedido-produto/${idPedido}/${idProduto}`);
      }

      // Produtos adicionados
      const adicionados = produtosNovos.filter((id) => !produtosAntigos.includes(id));
      for (const idProduto of adicionados) {
  ...
  const toggleProduto = (produto: any) => {
    setFormEdicao((prev) => {
      const jaExiste = prev.produtos.some((p) => p.idProduto === produto.idProduto);
      return {
        ...prev,
        produtos: jaExiste
          ? prev.produtos.filter((p) => p.idProduto !== produto.idProduto)
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {produtosDisponiveis.map((produto) => {
                        const selecionado = formEdicao.produtos.some((p) => p.idProduto === produto.idProduto);
                        return (
                          <button
